...
from ppg_functions import get_R_values, get_SpO2_values, time_sync
...
class validation():
    ...
    def __init__(self, coef, files, participants):

        self.sample_len = coef.sample_len
        self.ppg_fs = coef.ppg_fs
        self.spo2_fs = coef.spo2_fs
        self.sao2_fs = coef.sao2_fs

        error = []
        for filename in files:
            if filename in participants:
                ...
                if len(synced_R_values) > 0:
                    error = np.hstack((error,coef.A - coef.B*np.ndarray.flatten(np.transpose(synced_R_values))-synced_SpO2_values))
        ...
        '''
        plt.plot(coef.A - coef.B*synced_R_values,label='A - B * R')
        plt.plot(synced_SpO2_values,label='SaO2')
        plt.plot(coef.A - coef.B*synced_R_values-synced_SpO2_values,label='Error')
        plt.xlabel('seconds')
        plt.legend(loc='best')
        plt.show()
        '''
...
def time_sync( R_values, ppg_base_time, SpO2_values, SpO2_base_time):
    ...
    synced_R_values = np.array(synced_R_values)
